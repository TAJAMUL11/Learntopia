    ...
    const fetchData = async () => {
      ...
      try {
        ...
        quizSnap.forEach((docSnap) => {
          ...
          // Always add to totalPoints if there's a score
          if (data.score) {
            calculatedTotalPoints += data.score * 10;
          }
          ...
          // Best effort sync to global QuizLeaderboard
          const possibleQuizId = data.quizId || data.quizTitle?.toLowerCase().split(" ")[0]; 
          if (possibleQuizId) {
            const globalScoreRef = doc(db, "QuizLeaderboards", possibleQuizId, "Scores", currentUser.uid);
            setDoc(globalScoreRef, {
        ...
        // Sync to PublicLeaderboard for global read access
    ...
    <div className="container-page py-16 md:py-20 text-ink-hi">
      {loading ? (
        ...
        <div className="mx-auto max-w-5xl space-y-8 animate-fade-up">
          ...
          {/* Profile Header */}
          <Card className="flex flex-col items-start justify-between gap-6 p-6 sm:flex-row sm:items-center md:p-8 border-violet-500/20 bg-gradient-to-br from-violet-500/5 to-transparent">
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                {/* Light & Airy Streak & Badges Row */}
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-orange-500/25 bg-orange-500/10 px-2.5 py-0.5 text-xs font-semibold text-orange-300">
                    <span>🔥</span> {(Number(userDetails?.streak) || 1) === 1 ? "1 Day Streak" : `${Number(userDetails?.streak)} Days Streak`}
                  </span>

                  {userDetails?.badges && userDetails.badges.length > 0 && (
                    userDetails.badges.map((badge, idx) => (
                      <span key={idx} className="inline-flex items-center gap-1 rounded-full border border-sky/25 bg-sky/10 px-2.5 py-0.5 text-xs font-semibold text-sky">
